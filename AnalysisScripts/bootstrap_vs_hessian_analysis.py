...
"""Compare bootstrap widths with Hessian errors for the same data bins."""
...
from analysis_utils import (
    ...
    r_expression, read_tree, save_figure, style_axis, wave_label, wrap_phase,
)
...
SHOW_PLOTS = False
OUTPUT_DIR = PROJECT_DIR / "AnalysisScripts/outputs/bootstrap_vs_hessian"
Q2_VALUES = {
    "e_rho": np.array([0.82, 1.19, 1.66, 3.06]),
    "mu_rho": np.array([1.14, 1.60, 2.80, 6.02]),
    "e_omega": np.array([1.28, 2.00, 4.00]),
    "mu_omega": np.array([1.16, 1.64, 3.58]),
}
...
if PAIRS is None:
    PAIRS = []
    for bootstrap_file in sorted((PROJECT_DIR / "OutputFiles").glob("*_bootstrap_*.root")):
...
for bootstrap_file, fit_file in PAIRS:
    ...
    for names, ylabel, filename, phase in (
        ...
        bootstrap_value = np.array([np.mean(bootstrap[name]) for name in names])
        bootstrap_error = np.array([
            np.std(bootstrap[name], ddof=1) for name in names
        ])
