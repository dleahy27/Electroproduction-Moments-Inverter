// Produce a self-consistent synthetic moment bin from random complex waves.
// This is useful for closure tests: data generated by the model should be
// recoverable by a fit using the same wave and polarization assumptions.
...
void GenerateRandomMoments(const std::filesystem::path& output,
  ...
  const std::filesystem::path outPath = output.empty()
      ? std::filesystem::path("random_input_moments.root")
      : output;
  ...
  // The synthetic context uses the same constraints and moment definitions as
  // fitting, making generated samples suitable for direct closure tests.
  ...
  for (i = 0; i < static_cast<int>(nEvents); ++i) {
    // Every row is an independent amplitude draw and its calculated moment
    // vector. Unit errors are placeholders for downstream numerical tests.
